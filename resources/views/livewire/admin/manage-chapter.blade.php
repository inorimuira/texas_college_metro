                                ...
                                <div class="flex justify-between items-center px-4 py-3 hover:bg-gray-100 cursor-pointer rounded-md"
                                    ...
                                    <div class="flex space-x-2">
                                        ...
                                        @if ($chapter->nama_chapter != 'Placement Test')
                                            <button @click.stop
                                                wire:click="confirmDelete({{ $chapter->id }}, 'chapter')"
                                                ...
                                                @if ($chapter->nama_chapter != 'Placement Test')
                                                    <div class="flex space-x-2">
                                                        ...
                                                        <button
                                                            ...
                                                            class="text-red-500 hover:text-red-700 mr-2">
                                                            <x-icon-admin icon="iconDelete"
                                                                fill="#ef4444"></x-icon-admin>
                                                        </button>
                                                    </div>
                                                @endif
                                            </div>
                    ...
                    <form wire:submit.prevent="tambahChapter">
                        <div class="mb-4">
                            <label for="chapter_name"
                                class="block text-sm font-medium text-gray-700 mb-2 after:content-['*'] after:ml-0.5 after:text-red-500">Nama
                                Chapter</label>
                            <input type="text" wire:model="chapter_name"
                                placeholder="Nama chapter ex: Chapter 1 - Past Tense"
            ...
            @if ($selectedModule != null)
                <div x-show="isManagingModule" x-cloak class="bg-white shadow-md rounded-md p-6">
                    ...
                    <div class="flex justify-end">
                        <x-button-secondary type="button" iconNone="true"
                            @click="isPopupEditSummary = !isPopupEditSummary">Edit Summary</x-button-secondary>
                    </div>
                    ...
                    @if ($selectedModule && $selectedModule->activityModule->isNotEmpty())
                        <div x-data="{
                            ...
                            @foreach ($selectedModule->activityModule as $activity)
                                <div class="flex justify-between items-center px-4 py-2 hover:bg-gray-100 cursor-pointer"
                                    ...
                                    <div class="flex space-x-2" @click.stop>
                                        ...
                                        <button wire:click="confirmDelete({{ $activity->id }}, 'aktivitas modul')"
                                            class="text-red-500 hover:text-red-700 mr-2">
                                            <x-icon-admin icon="iconDelete" fill="#ef4444"></x-icon-admin>
                                        </button>
                                    </div>
                                ...
                                <div x-show="openActivities['{{ $activity->id }}']" x-collapse x-cloak>
                            ...
                            <div class="flex justify-center py-3">
                                <x-button-primary @click="isPopupOpenActivity = true" iconNone="true"
                                    class="text-sm">
                                    Tambah Aktivitas
                                </x-button-primary>
                            </div>
            ...
            <div x-show="isPopupEditSummary" x-cloak
                ...
                <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-5 relative"
                    ...
                    <form wire:submit.prevent="editSummary({{ $selectedModule ? $selectedModule->id : '' }})">
                        <div class="mb-4">
                            <label for="summary"
                                class="block text-sm font-medium text-gray-700 mb-2 after:content-['*'] after:ml-0.5 after:text-red-500">Summary</label>
                            @error('summary')
            ...
            <div x-show="isPopupOpenActivity"
                ...
                <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-5 relative">
                    ...
                    <form wire:submit.prevent="tambahActivity({{ $selectedModule ? $selectedModule->id : '' }})">
                        <div class="mb-4">
                            <label for="type"
                                class="block font-semibold mb-2 after:content-['*'] after:ml-0.5 after:text-red-500">Pilih
                                Aktivitas</label>
                            <select wire:model="type" class="w-full p-2 border border-gray-300 rounded"
                        ...
                        <template x-if="selectedActivity === 'video'">
                            <div class="space-y-2">
                                <div>
                                    <label for="judul_video"
                                        class="block font-semibold mb-2 after:content-['*'] after:ml-0.5 after:text-red-500">Judul
                                        Video</label>
                                    @error('judul_video')
                                        <p class="text-red-500 text-xs mt-1 mb-1">{{ $message }}</p>
                                    @enderror
                                    <input wire:model="judul_video" type="text"
                                        class="w-full p-2 text-gray-700 border border-gray-300 rounded"
                                        placeholder="Masukkan judul video">
                                </div>
                                <div>
                                    <label for="link_video"
                                        class="block font-semibold mb-2 after:content-['*'] after:ml-0.5 after:text-red-500">Link
                                        Video</label>
                                    @error('link_video')
                                        <p class="text-red-500 text-xs mt-1 mb-1">{{ $message }}</p>
                                    @enderror
                        ...
                        <template x-if="selectedActivity === 'reading'">
                            <div class="space-y-2">
                                <div>
                                    <label for="judul_reading"
                                        class="block font-semibold mb-2 after:content-['*'] after:ml-0.5 after:text-red-500">Judul
                                        Aktivitas</label>
                                    @error('judul_reading')
                                        <p class="text-red-500 text-xs mt-1 mb-1">{{ $message }}</p>
                                    @enderror
                                    <input wire:model="judul_reading" type="text"
                                        class="w-full p-2 text-gray-700 border border-gray-300 rounded"
                                        placeholder="Masukkan judul aktivitas">
                                </div>
                                <div>
                                    <label for="text"
                                        class="block font-semibold mb-2 after:content-['*'] after:ml-0.5 after:text-red-500">Text</label>
                                    @error('text')
                                        <p class="text-red-500 text-xs mt-1 mb-1">{{ $message }}</p>
                                    @enderror
                                    <textarea wire:model="text" rows="5" class="w-full p-2 text-gray-700 border border-gray-300 rounded"
