...
<div class="grid gap-8" x-data="{ isAttendanceOpen: @entangle('isAttendanceOpen') }" >
    <div
    @if ($type == 'placement')
    ...
    @if ($type == 'course' && $presensi)
        <div class="p-6 bg-teal-200 text-white rounded-lg shadow-md flex justify-between">
            <div class="flex flex-col">
                ...
                <span class="text-slate-700">{{ $presensi->module->nama_module }}</span>
            </div>
            <div class="flex flex-col">
                    @if ($attendance)
                        <x-button-primary type="submit" iconNone="true" class="bg-highlight" wire:click="markAttendance({{ $presensi->id }}, {{ $presensi->module->id }})">
                            Attendance
                        </x-button-primary>
